...
def main() -> None:
    ...
    scenario_selection_group.add_argument(
        "-c",
        "--category",
        help="Run all scenarios in a specific category.",
        choices=[str(cat.value) for cat in ScenarioCategories],
        required=False,
    )
    parser.add_argument(
        "--no-augmentations",
        action="store_true",
        help="When used with --category, only run scenarios with no tool augmentations (NO_DISTRACTION_TOOLS).",
    )
    parser.add_argument(
        "-p",
        "--parallel",
    ...
    # Validate that --no-augmentations is only used with --category
    if args.no_augmentations and not args.category:
        parser.error("--no-augmentations can only be used with --category")

    # The parser for `--test_mode`, `--scenarios`, and `--category` are in a mutually exclusive group
    # so we can safely ignore the values when one of them is true.
    if args.test_mode:
