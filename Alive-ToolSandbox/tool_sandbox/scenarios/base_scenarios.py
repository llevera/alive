...
def named_base_scenarios(preferred_tool_backend: ToolBackend) -> Dict[str, Scenario]:
    ...
    # Read domain schema content
    domain_schema_content = ""
    try:
        with open("domain_schema.txt", "r", encoding="utf-8") as f:
            domain_schema_content = f.read()
    except FileNotFoundError:
        # If domain_schema.txt is not found, continue without it
        pass
    ...
    # Add domain schema content if available
    if domain_schema_content:
        sandbox_rows.append({
            "sender": RoleType.SYSTEM,
            "recipient": RoleType.AGENT,
            "content": domain_schema_content,
        })
    
    # Add few-shot examples
    sandbox_rows.extend(user_simulator_few_shot_examples[
